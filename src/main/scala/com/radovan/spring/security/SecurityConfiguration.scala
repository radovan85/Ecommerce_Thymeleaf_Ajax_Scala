...
class SecurityConfiguration extends WebSecurityConfigurerAdapter {
  ...
  override protected def configure(http: HttpSecurity): Unit = {
    ...
    http.authorizeRequests.antMatchers("/login").anonymous.antMatchers("/loginErrorPage", "/products/**", "/aboutUs", "/suspensionPage").permitAll.antMatchers("/", "/home", "/registerComplete", "/registerFail", "/loginPassConfirm", "/suspensionChecker").permitAll.antMatchers("/get*").hasAnyAuthority("ADMIN", "ROLE_USER").antMatchers("/admin/**").hasAuthority("ADMIN").antMatchers("/cart/**", "/order/**").hasAuthority("ROLE_USER").antMatchers("/userRegistration").anonymous.antMatchers("/register").anonymous.anyRequest.authenticated
  }
